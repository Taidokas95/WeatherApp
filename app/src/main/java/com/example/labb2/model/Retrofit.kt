...
val link1 = "https://maceo.sth.kth.se/weather/forecast?lonLat=lon/14.333/lat/60.383"
val link2 = "https://opendata-download-metfcst.smhi.se/api/category/pmp3g/version/2/geotype/point/lon/16/lat/58/data.json"
...
fun getWeathersFromDatabase(localWeathersState: WeathersState) = GlobalScope.launch(Dispatchers.Default) {
    ...
    //= WeathersState(latitude = 60.383f, longitude = 14.333f )
    ...
    /*call.enqueue(object : Callback<TheTestWeather> {
        override fun onResponse(call: Call<TheTestWeather>, response: Response<TheTestWeather>) {
            if (response.isSuccessful) {
                val post = response.body()
                if (post != null) {
                    println("Approved time = ${post.approvedTime}")
        ...
        override fun onFailure(call: Call<TheTestWeather>, t: Throwable) {
            println("Error: ${t.message}")
        }
    })*/
}
